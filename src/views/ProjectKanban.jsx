...
// ── FIX: use useParams correctly — route is /admin/projects/:id
...
export default function ProjectKanban() {
  // ── FIX: param name must match route definition ───────────────
  // Route: /admin/projects/:id  →  useParams gives { id }
  // Route: /admin/projects/:projectId  →  useParams gives { projectId }
  ...
  const handleDrop = async (stageKey) => {
    ...
    try {
      const res = await updateContent(draggingItem._id, { stage: stageKey });
  ...
  const handleSave = async () => {
    ...
    try {
      if (editTarget) {
        const res = await updateContent(editTarget._id, form);
  ...
  const handleProjSave = async () => {
    try {
      await updateProject(projectId, projForm);
      ...
      <Box sx={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", mb:3, flexWrap:"wrap", gap:2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {project?.clientId?.businessName || ""}
            {" · "}
            {postedContent}/{totalContent} content posted
          </Typography>
          {project?.monthlyGoal && (
        ...
        <Box sx={{ display:"flex", gap:1, flexWrap:"wrap" }}>
          ...
          <Button size="small" variant="outlined" color="secondary"
            ...
            onClick={() => navigate(`/admin/projects/${projectId}/shoot`)}>
            Shoot Schedule
          </Button>
      ...
      <Dialog open={dialog} onClose={() => setDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <Grid container spacing={2} sx={{ mt:0.5 }}>
            ...
            <Grid item xs={12}>
              <TextField {...f("description")} label="Caption / Concept" multiline rows={2}
            ...
            {editTarget && (
              <Grid item xs={12}>
                <TextField {...f("instagramLink")} label="Instagram Link (after posting)"
